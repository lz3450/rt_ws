...
def log_to_csv(log_file: str, output_file: Optional[str] = None):
    ...
    # get init time
    filtered_logs = logs_df[(logs_df['node_name'] == 'turtle2_controller') & (logs_df['message'] == '"turtle2" successfully spawned.')]
...
def check_0(kd: float, ka: float, time_limit: float, distance_limit: float) -> dict[str, Any]:
    ...
    result_distance['vel_lower_bound'] = ((distance_df['value'][1:] - distance_limit).apply(lambda x: x if x > 0 else 0) / (time_limit - distance_df['timestamp'].apply(lambda x: x if x < time_limit else np.NaN)))
    ...
    vel_lower_bound_timestamp_check_success = result_distance["timestamp"][result_distance[result_distance['vel_lower_bound'] > 4.0].index[0]] if not vel_lower_bound_check_success else 0

    ctrl_error_check_success = (result_distance['ctrl_error'][result_distance["timestamp"] <= TIME_LIMIT] > CTRL_ERROR_THRESHOLD).sum() < CTRL_ERROR_OVER_THRESHOLD_LIMIT
    ...
    ax.legend()
    title_suffix = f'(Failed at {vel_lower_bound_timestamp_check_success:.4f})' if not vel_lower_bound_check_success else '(Success)'
...
def check_1(kd: float, ka: float, time_limit: float, distance_limit: float) -> dict[str, Any]:
    log_csv_file = f'../log/myturtlesim_tf2_launch_{str(kd).replace('.', '')}_{str(ka).replace('.', '')}.csv'
    result_csv_file = f'../log/myturtlesim_tf2_launch_{str(kd).replace(".", "")}_{str(ka).replace(".", "")}_check_1.csv'
    ...
    def _predict(ed: float, ea: float, kd: float, ka: float, delta_ts: pd.Series) -> pd.DataFrame:
        ...
        result_df = pd.DataFrame(result, index=delta_ts.index - 1)
        return result_df

    results_df = df.copy()
    results_df['objective'] = results_df.apply(lambda x: objective_function(x['distance'], x['timestamp'], time_limit, distance_limit), axis=1)
    ...
    for row in df.itertuples():
        ...
        check_success = pd.concat([df['timestamp'], result_df], axis=1).apply(lambda x: objective_function(x['predicted_distance'], x['timestamp'], time_limit, distance_limit), axis=1).all()
    ...
    for i in prediction_starting_indices:
        ax.plot(results_df['timestamp'], results_df[f'predicted_distance_{i}'], label=f'predicted_distance_{i}', marker='.')
    ax.legend(fontsize=8, loc='upper right')
    ax.set_title(f'kd = {kd}, ka = {ka} Actual: {actual_success} Predicted: {check_success}')
    fig.tight_layout(w_pad=0, h_pad=0)
    fig.savefig(f'../log/myturtlesim_tf2_launch_{str(kd).replace(".", "")}_{str(ka).replace(".", "")}_check_1.png', )
    plt.close(fig)
...
def run(i: int, j: int, results: list, check_func: Callable):
    filename_prefix = f'../log/myturtlesim_tf2_launch_{i+5:02d}_{j+5:02d}'
    log_file = f'{filename_prefix}.log'
    csv_file = f'{filename_prefix}.csv'
    if os.path.exists(log_file) and (not os.path.exists(csv_file) or os.path.getmtime(log_file) > os.path.getmtime(csv_file)):
        log_to_csv(log_file)
...
if __name__ == '__main__':
    ...
    if check_func is check_0:
        ...
        vel_lower_bound_check_missing_cases = check_results_df[check_results_df['vel_lower_bound'] != check_results_df['success']]
        ...
        check_success_missing_cases = check_results_df[check_results_df['check_success'] != check_results_df['actual_success']]
